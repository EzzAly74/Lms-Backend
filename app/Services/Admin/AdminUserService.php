...
class AdminUserService
{
    ...
    /** @var array<int,string> Roles displayed by the Figma role badge. */
    public const ROLES = ['admin', 'instructor', 'learner'];
        ...
        if ($search) {
            ...
            $query->where(function ($w) use ($needle) {
                $w->where('p.name_en', 'LIKE', $needle)
                  ...
                  ->orWhere('p.job_title', 'LIKE', $needle);
            });
        }
     ...
     * supporting lookups.
     *
     *   roles        →  every persona supported by the API (drives the
     *                   "Role" dropdown in the Add/Edit modal).  Each entry
     *                   carries the canonical key, the human label, and the
     *                   live count of rows in the underlying table so the
     *                   UI can render the picker dynamically — adding a new
     *                   persona only requires extending self::ROLES here.
     *   instructors  →  every row in the `instructors` table (the
     *                   Instructors filter sub-modal).
     *   job_titles   →  unique job-title strings across all three tables;
     *                   used as datalist hints in the Add/Edit modal.
     *
     * @return array{
     *   roles:       array<int,array{key:string,label:string,count:int}>,
     *   instructors: array<int,array{id:int,name:string,email:string|null}>,
     *   job_titles:  array<int,string>
     * }
     */
    public function filterOptions(): array
    {
        ...
        // Persona dropdown — counts come from whichever table backs each
        // role so the admin can see the totals at a glance.
        $roles = [];
        foreach (self::ROLES as $role) {
            $roles[] = [
                'key'   => $role,
                'label' => $this->labelForRole($role, $locale),
                'count' => (int) DB::table($this->tableFor($this->sourceForRole($role)))->count(),
            ];
        }
        ...
        foreach (['users', 'instructors', 'admins'] as $table) {
            if (Schema::hasColumn($table, 'job_title')) {
                $titles = $titles->concat(DB::table($table)
        ...
        return [
            'roles'       => $roles,
            'instructors' => $instructors,
            'job_titles'  => $titles->unique()->sort()->values()->all(),
        ];
    }

    /**
     * Map a persona key to its corresponding `source` token.
     */
    ...
    /**
     * Translate a persona key into a user-facing label, honouring the
     * current Accept-Language locale so the Add/Edit modal renders
     * correctly in both English and Arabic.
     */
    private function labelForRole(string $role, string $locale): string
    {
        $labels = [
            'en' => ['admin' => 'Admin',  'instructor' => 'Instructor', 'learner' => 'Learner'],
            'ar' => ['admin' => 'مدير',   'instructor' => 'مُدرّب',      'learner' => 'متدرّب'],
        ];
     ...
     *   phone?:string|null, learner_type?:string|null
     * }  $data
     *
    ...
    public function create(array $data): array
    {
        ...
        return DB::transaction(function () use ($data, $role) {
            return match ($role) {
                ...
                'instructor' => $this->createInstructor($data),
                default      => $this->createLearner($data),
            };
        });
    }

    private function createLearner(array $data): array
    {
    ...
    private function createInstructor(array $data): array
    {
    ...
    {
        // Admin's $fillable list intentionally omits status / job_title to
        ...
        if (Schema::hasColumn('admins', 'job_title')) { $admin->job_title = $data['job_title'] ?? null; }
        $admin->save();
    ...
    public function update(string $source, int $id, array $data): \stdClass
    {
        ...
        DB::transaction(function () use ($source, $id, $data) {
            match ($source) {
                'user'       => $this->updateLearner($id, $data),
                'instructor' => $this->updateInstructor($id, $data),
                'admin'      => $this->updateAdmin($id, $data),
            };
        });
    ...
    private function updateLearner(int $id, array $data): void
    {
        $user = User::query()->findOrFail($id);

        $payload = collect($data)
            ->only(['name_en', 'name_ar', 'email', 'phone', 'job_title',
    ...
    private function updateInstructor(int $id, array $data): void
    {
        ...
        foreach (['email', 'job_title', 'status'] as $key) {
    ...
    private function updateAdmin(int $id, array $data): void
    {
        ...
        if (array_key_exists('job_title', $data) && Schema::hasColumn('admins', 'job_title')) {
    ...
    private function unifiedQuery(): QueryBuilder
    {
        ...
        $adminHasJobTitle     = Schema::hasColumn('admins',      'job_title');
        ...
        $adminsSub = DB::table('admins')
            ...
            ->selectRaw('NULL AS machine_code')
            ->selectRaw('NULL AS department_name')
            ->selectRaw($adminHasJobTitle ? 'job_title AS job_title' : 'NULL AS job_title')
